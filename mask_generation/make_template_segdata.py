...
if __name__ == '__main__':
    ...
    # Analysis parameters from Blender CLI
    argv = sys.argv
    if "--" in argv:
    ...
    parser.add_argument("--data_type", type=str, choices=['amodal', 'plant', 'komatsuna', 'ara'], default='ara')
    parser.add_argument("--output_dir", type=str, default='../plant_data')
    ...
    gen_time = len(os.listdir(OUTPUT_PATH)) + 1
    os_dir = os.path.join(OUTPUT_PATH, f'data_{str(gen_time).zfill(5)}_{DATA_TYPE}')
    root_dir = os_dir
    os.mkdir(os_dir)
    ...
    if args.radius is not None:
        ...
        if DATA_TYPE == 'amodal':
            ...
            RADIUS = 0
        elif DATA_TYPE in ['ara', 'komatsuna']:
            RADIUS = 0.1
        else:
            RADIUS = 25
    DATA_NUM = args.data_num

    # Settings about lsystem and blender
    obj_name = "Cube"

    # meshes_name_list = ["NewMesh"]
    meshes_name_list = ["Cube", 'lsystem', 'lsystem-tmp', "NewMesh"]

    for obj_name in meshes_name_list:
        if obj_name in bpy.data.objects:
            obj = bpy.data.objects[obj_name]
            bpy.data.objects.remove(obj, do_unlink=True)
            print(f"{obj_name} has been removed.")
        else:
            print(f"{obj_name} does not exist.")

    int_list = [i for i in range(1, 1000001)]
    seed_list = random.sample(int_list, DATA_NUM)
    if DATA_TYPE in ['plant', 'amodal']:
    ...
    for idx, seed in enumerate(seed_list):
        if DATA_TYPE == 'komatsuna':
            ...
            lsys = create_lsystem(DATA_TYPE, leaf_dict)
        main(lsys, seed, DATA_TYPE, idx, CAM_NAME, os_dir, RESOLUTION, RENDER_NUM, RADIUS)